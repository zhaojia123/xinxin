...
func (s *Store) EmploymentChanges(ctx context.Context) ([]response.EmploymentChange, error) {
	...
	rows, err := s.DB.QueryContext(ctx, `SELECT c.id,e.employee_no,e.name,c.change_type,CONCAT_WS(' / ',NULLIF(bd.name,''),NULLIF(bp.name,''),NULLIF(CASE c.before_status WHEN 'probation' THEN '试用期' WHEN 'active' THEN '在职' WHEN 'left' THEN '已离职' ELSE '' END,'')),CONCAT_WS(' / ',NULLIF(ad.name,''),NULLIF(ap.name,''),NULLIF(CASE c.after_status WHEN 'probation' THEN '试用期' WHEN 'active' THEN '在职' WHEN 'left' THEN '已离职' ELSE '' END,'')),DATE_FORMAT(c.effective_on,'%Y-%m-%d'),c.reason,COALESCE(u.display_name,'系统') FROM employment_changes c JOIN employees e ON e.id=c.employee_id LEFT JOIN departments bd ON bd.id=c.before_department_id LEFT JOIN positions bp ON bp.id=c.before_position_id LEFT JOIN departments ad ON ad.id=c.after_department_id LEFT JOIN positions ap ON ap.id=c.after_position_id LEFT JOIN admin_users u ON u.id=c.operator_id ORDER BY c.effective_on DESC,c.id DESC LIMIT 500`)
	if err != nil {
