...
func (s *Store) Positions(ctx context.Context) ([]response.Position, error) {
	...
	rows, err := s.DB.QueryContext(ctx, `SELECT p.id,p.department_id,p.position_no,p.name,d.name,p.level_name,p.min_salary,p.max_salary,p.enabled,p.remark,COUNT(e.id) FROM positions p JOIN departments d ON d.id=p.department_id LEFT JOIN employees e ON e.position_id=p.id AND e.employment_status<>'left' GROUP BY p.id,p.department_id,p.position_no,p.name,d.name,p.level_name,p.min_salary,p.max_salary,p.enabled,p.remark,p.sort_order ORDER BY d.sort_order,p.sort_order,p.id`)
	if err != nil {
...
func (s *Store) OrganizationSummary(ctx context.Context) (response.OrganizationSummary, error) {
	...
	err := s.DB.QueryRowContext(ctx, `SELECT (SELECT COUNT(*) FROM departments WHERE enabled=1),(SELECT COUNT(*) FROM positions WHERE enabled=1),(SELECT COUNT(*) FROM employees WHERE employment_status IN ('active','probation'))`).Scan(&v.DepartmentCount, &v.PositionCount, &v.ActiveEmployeeCount)
	if err != nil {
